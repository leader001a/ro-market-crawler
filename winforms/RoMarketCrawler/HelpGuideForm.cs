...
- 서버: 모니터링 서버
- 완전일치: 체크 시 아이템명이 정확히 일치하는 결과만 표시
- 아이템: 모니터링 중인 아이템
- 감시가: 설정한 목표 가격 (더블클릭으로 수정)
- 상태: 자동갱신 시 남은 시간 표시


[완전일치 기능]

완전일치를 체크하면 입력한 아이템명과 정확히 일치하는
결과만 표시됩니다.

예시:
- ""포링 카드"" 검색 시:
  - 완전일치 OFF: 포링 카드, 수정 포링 카드 등 모두 표시
  - 완전일치 ON: ""포링 카드""만 표시

유사한 이름의 아이템이 많을 때 원하는 아이템만
정확히 모니터링하고 싶을 때 사용하세요.
...
[완전일치 기능]
- 체크 시 아이템명 정확히 일치하는 결과만 표시
- 예: ""포링 카드"" → 수정 포링 카드 등 제외
