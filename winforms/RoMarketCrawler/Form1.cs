...
의상검색 탭에서 ""자동 수집"" 버튼을 누르면, 수집 완료 후 5분마다 자동으로 다시 수집합니다. 두 번째 수집부터는 이전에 수집한 아이템의 상세정보를 기억하고 있어, 새로 올라온 아이템만 상세정보를 요청합니다. 덕분에 첫 수집은 약 54분이 걸리지만, 이후에는 약 6분이면 전체 갱신이 완료됩니다.
...
의상 노점 검색을 웹 사이트로 제공하려면, 서버 한 대가 gnjoy에 주기적으로 요청을 보내서 데이터를 수집해야 합니다. 그런데 위에서 설명한 것처럼 첫 수집에만 약 54분이 걸리고, 이후에도 약 6분이 필요합니다. 여기에 사용자 수만큼 서버 수집량이 늘어나면 차단 위험이 급격히 높아집니다.
