...
의상 노점 검색을 웹 사이트로 제공하려면, 서버 한 대가 gnjoy에 주기적으로 요청을 보내서 데이터를 수집해야 합니다. 그런데 위에서 설명한 것처럼 의상 1,000개 기준 **한 번 수집에만 약 25분**이 걸립니다.

만약 30분~1시간마다 최신 데이터로 갱신하려고 하면, **이전 수집이 끝나기도 전에 다음 수집을 시작해야 하는 상황**이 됩니다. 요청 간격을 줄이면 차단당하고, 간격을 유지하면 갱신이 안 되는 딜레마에 빠지게 됩니다.
